...
let FootprintFormComponent = class FootprintFormComponent {
    ...
    calcFactors() {
        const csvData = this.CSVParse();
        const country = this.firstFormGroup.get('firstCtrl').value;
        const coal = (csvData[country]).Coal;
        const gas = (csvData[country]).Gas;
        const oil = (csvData[country]).Oil;
        const hydro = (csvData[country]).Hydro;
        const renew = (csvData[country]).Renewable;
        const nuclear = (csvData[country]).Nuclear;
        const power = this.secondFormGroup.get('secondCtrl').value / 1000;
        ...
        const totalPercentage = Number(coal) + Number(gas) + Number(oil) + Number(hydro) + Number(renew) + Number(nuclear);
        const totalPercentageString = Math.round(totalPercentage * 10) / 10 + '%';
        const kgCO2result = ((coal * 820 + gas * 490 + oil * 778 +
...
FootprintFormComponent.ctorParameters = () => [
